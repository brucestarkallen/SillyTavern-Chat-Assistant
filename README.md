...
- **Director** — writes secret per-episode directives (hidden `[EPISODE_END]` markers) that give NPCs and the world their own initiative and give pacing an arc, injected into your storyteller so the world acts *on* the protagonist. Three modes: **Auto** (AI invents and chains episodes on its own), **Co-writer** (each episode grows from *your* one-line seed via `#e`/🎬 Seed — the AI drafts the hidden beats around it, and 💡 Seed ideas proposes three doors when you're blank), and **Off** (manual buttons only). Peek/edit/`#d` work in every mode.
