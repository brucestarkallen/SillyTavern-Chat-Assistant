...
(() => {
    ...
    const SHOWRUNNER_PASS_PROMPT = [
        ...
        '5. LOGIC \u2014 does every element obey the world\'s own rules, established canon, and plausible causation \u2014 would a skeptical viewer accept why each beat happens now, this way? And does it keep the format contract (intensity line; premise with EPISODE QUESTION; beats with a TURN and a final DILEMMA; NPC & world initiative; open landing with per-answer consequences; hook; arc)? Fix what breaks. Secrecy stays intact.',
        '6. SOVEREIGNTY \u2014 hunt every sentence that decides FOR the player: a beat where the player character acts, speaks, chooses, or feels; a landing or hook that assumes which way the EPISODE QUESTION resolves; a consequence that exists only on one unchosen branch. The plan pressures; only the player answers. Rewrite each violation to stop at the decision point and carry consequences for any answer \u2014 this outranks boldness: a thrilling draft that steals the player\'s choice is a failed draft.',
        'Deliberate efficiently \u2014 the token budget is shared between private reasoning and the final directive; settle your six interrogations decisively and spend the tokens on the improved cut itself.',
    ...
    function loadSettings() {
        ...
        const legacyPrompts = [LEGACY_DIRECTOR_PROMPT, LEGACY_DIRECTOR_PROMPT_V240.replace(HOOK_LINE, HOOK_LINE_2370), LEGACY_DIRECTOR_PROMPT_V240, LEGACY_DIRECTOR_PROMPT_V241, LEGACY_DIRECTOR_PROMPT_V246, LEGACY_DIRECTOR_PROMPT_V248, LEGACY_DIRECTOR_PROMPT_V251, LEGACY_DIRECTOR_PROMPT_V252, LEGACY_DIRECTOR_PROMPT_V257, LEGACY_DIRECTOR_PROMPT_V262];
