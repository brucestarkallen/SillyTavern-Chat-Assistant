...
- **Ghosted originals stay fetchable.** `fullTextOf()` never filters on `is_system` — the audit's `<verify>` pass and ordinary doubt-fetching both depend on reading a ghosted original whole. What is forbidden is *unhiding* ghosted messages, never reading them; the full-text window skips them (v2.80) precisely because fetch remains the on-demand path.
...
- **A fix is a class, not an instance.** The same fact lives in chat prose, memory snippets, snippet detail fields, ledger dossiers, the notepad and the worldbook. `CONSISTENCY_LAW` ships on every request (not only audits) and `rippleScan()` proves the leftovers in code before staging. Any new surface that stores story facts must be added to the scan, or fixes will silently stop reaching it.
- **Every model call must be counted.** `auditAsk` takes a `stats` object and increments it per call, including the anchor and ripple correction rounds — rounds spent inside it were once invisible to the audit budget, so a 40-call budget could quietly spend 80.
